...
export class Animal {
  ...
  public idleAnims:AnimationState[] = new Array();
  public attackAnim:AnimationState;
  public walkAnim:AnimationState;
  public runAnim:AnimationState;

  public currentClip:AnimationState;
  ...
  constructor(_name:string, _modelFile:string, _animPrefix:string, _idleClipNames:string[], _position: Vector3, _scale: Vector3) {
    ...
    this.ent = new Entity();
    this.ent.addComponent(new Transform({ position: _position }));
  ...
  loadModel(_filename: string, _idleClipNames:string[], _position: Vector3, _scale: Vector3) {
    ...
    this.shape = new GLTFShape("models/animals/" + _filename);
    log("Loading file: models/animals/" + _filename);
    mod.addComponent(this.shape);
    mod.addComponent(new Transform({ scale: _scale}));
    const anim = new Animator();
    mod.addComponent(anim);
    ...
    let newClip:AnimationState;
    for (var i:number = 0; i < _idleClipNames.length; i++)
    {
        newClip = new AnimationState(this.animPrefix + _idleClipNames[i], {looping: true, speed: 0.8});
        newClip.weight = 0;
        anim.addClip(newClip);
        this.idleAnims.push(newClip);
        log("Added idle animation: " + this.animPrefix + _idleClipNames[i]);
    }

    this.walkAnim = new AnimationState(this.animPrefix + "walk", {looping: true});
    this.walkAnim.weight = 0;
    anim.addClip(this.walkAnim);

    this.runAnim = new AnimationState(this.animPrefix + "run", {looping: true});
    this.runAnim.weight = 0;
    anim.addClip(this.runAnim);

    this.attackAnim = new AnimationState(this.animPrefix + "attack", {looping: false});
    this.attackAnim.weight = 0;
    anim.addClip(this.attackAnim);
  ...
  getShape() 
  {
    return this.modelEntity.getComponentOrNull(GLTFShape);
  }
  ...
  planMovement()
  {
    let trans:Transform = this.ent.getComponent(Transform);
    this.origin = trans.position;
    
    // Pick a destination, then walk there
    this.destination = this.chooseDestination();

    // DONE: Orient the animal toward the destination -- Vector3.lookAt() did not work as intended
    var angles:Vector3 = trans.rotation.eulerAngles;
    angles.y = Math.atan2(this.destination.x - this.origin.x, this.destination.z - this.origin.z)*180 / Math.PI;
    this.ent.getComponent(Transform).rotation.eulerAngles = angles;
    
    this.distance = Vector3.Distance(this.ent.getComponent(Transform).position, this.destination);
    this.traveled = 0;
    this.fraction = 0;
  ...
  /**
   * This delay is only needed because animation clips tend to blend together if different ones are
   * stopped and started during the same fram.
   */
  pauseToThink()
  {
    this.stopCurrentClip();
  ...
  {
      let pos:Vector3 = this.ent.getComponent(Transform).position;
  ...
  move(dt: number)
  {
    ...
    this.ent.getComponent(Transform).position = newPos;
  }
}